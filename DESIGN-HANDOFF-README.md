...
## The Connect Lite identity (specs/design_handoff_connect_lite_logo, 20 Sep 2026)
The Lite mark is the Connect mark's two Cs standing apart (viewBox `8 30 126 60`, second C at x 118.1; the parent's is `8 30 104 60`). `brand/` holds the assets cut from the sheet: `mark.svg`, `mark-reversed.svg`, `mark-mono.svg`, `mark-header.svg` (stroke 12), `avatar.svg` (84px tile, stroke 13), `favicon.svg` (28px tile, stroke 15), plus `favicon.ico` (16 + 32) and `apple-touch-icon.png` (180) rendered from the favicon geometry; every page links the three. Built from the spec: **configuration A** in the header of the home, tutor dashboard, course admin and assessor pack (bare mark 56×27 at stroke 12, "Connect" Instrument Serif Italic 20px gold, "Lite" Instrument Sans Regular 9px uppercase 0.22em matte black; `.lite-lockup`, `.wordmark`, `.wordmark-connect`, `.wordmark-lite` in `hub-shared.js`); **configuration F** (single colour, first arc at 45%) as a stamp on the right of every printed record's letterhead (`hubLiteStampHTML`), the centre on the left; no eyebrow under the lockup (Ramy: "lose the eyebrow — it's not just about teaching practice any more"; the two "Teaching practice paperwork" page eyebrows on screens 1 and 3 went with it). Not built, because the spec was written for an offline file build and Lite is a store-backed site: the repo rename to `connect-lite` (the Pages URL is every link in circulation), self-hosted fonts, a launch screen (configuration D has no surface), the footer wordmark (the credit lives in the header by the 20 Sep ruling), the Connect-side folder label, and the app-wide switch to Instrument Sans / IBM Plex Mono (the 20 Sep ruling kept Newsreader + Karla for the pages).
